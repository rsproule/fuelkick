...
abigen!(Contract(
    name = "FuelKick",
    abi = "out/debug/fuelkick-abi.json"
));

const TICTACTOE_CONTRACT_BINARY_PATH: &str = "./out/debug/fuelkick.bin";
const TICTACTOE_CONTRACT_STORAGE_PATH: &str = "./out/debug/fuelkick-storage_slots.json";

pub struct Player {
    pub contract: FuelKick,
...
pub async fn setup() -> (Player, Player) {
    ...
    let player_one = Player {
        contract: FuelKick::new(id.clone(), player_one_wallet.clone()),
        identity: identity(player_one_wallet.address()).await,
    };

    let player_two = Player {
        contract: FuelKick::new(id, player_two_wallet.clone()),
